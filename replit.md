...
- June 13, 2025. Enhanced Character Command Interface - Added intuitive quick action commands (Chat, Interact, Affection, Training, Mission) with color-coded icons above thought prompts. Commands auto-fill text input with appropriate phrases for faster interaction. Maintains dual-layer system with quick commands and dynamic contextual prompts.
- June 13, 2025. Dialogue Interface Layout Fix - Fixed conversation container to use 270px fixed height preventing input controls from being pushed off screen. Updated Hunter's Communicator to display Cha Hae-In's actual avatar image instead of emoji in both conversation list and chat header. Improved modern messaging app behavior with proper scrolling and fixed input positioning.
