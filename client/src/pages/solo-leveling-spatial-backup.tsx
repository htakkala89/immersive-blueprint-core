...
export default function SoloLevelingSpatial() {
  ...
  // Episode system functions
  const episodeAvailabilityCheck = useCallback(async () => {
    if (!gameState || !gameState.level) return;
    
    // Check for new episode availability based on story progress
    const currentStoryProgress = gameState.storyProgress || 0;
    const currentLevel = gameState.level;
    
    const newlyAvailable = AVAILABLE_EPISODES.filter(episode => {
      const isUnlocked = episode.requirements.level <= currentLevel && 
                        episode.requirements.storyProgress <= currentStoryProgress;
      const alreadyNotified = episodeNotifications.some(n => n.episodeId === episode.id);
      return isUnlocked && !alreadyNotified;
    });
    
    if (newlyAvailable.length > 0) {
      setAvailableEpisodes(prev => [...prev, ...newlyAvailable]);
      
      // Create notifications for new episodes
      const notifications = newlyAvailable.map(episode => ({
        id: `episode_${episode.id}`,
        episodeId: episode.id,
        message: `New Episode Available: ${episode.title}`,
        timestamp: Date.now()
      }));
      
      setEpisodeNotifications(prev => [...prev, ...notifications]);
      setNotifications(prev => [...prev, ...notifications.map(n => ({
        id: n.id,
        type: 'episode_available' as const,
  ...
  }, [gameState, episodeNotifications]);

  const startEpisode = useCallback((episodeId: string) => {
    const episode = availableEpisodes.find(ep => ep.id === episodeId);
    if (!episode) return;
    
    if (gameState.level < episode.requirements.level) {
      // Show level requirement message
      return;
    }
    
    if ((gameState.storyProgress || 0) < episode.requirements.storyProgress) {
      // Show story progress requirement message  
      return;
    }
  ...
  const triggerAutoDateScene = (dateInfo: any) => {
