...
export function DungeonRaidSystem11({ 
  ...
  // Core game state
  const [gamePhase, setGamePhase] = useState<'intro' | 'combat' | 'puzzle' | 'boss_antechamber' | 'room_clear' | 'complete'>('intro');
  const [currentRoom, setCurrentRoom] = useState(1);
  const [totalRooms] = useState(7);
  const [dungeonAct, setDungeonAct] = useState(1); // Act I: 1-2, Act II: 3-5, Act III: 6-7
  ...
  const generateRoomEnemies = useCallback((room: number, act: number) => {
    ...
    return Array.from({ length: enemyCount }, (_, i) => ({
      ...
      x: 300 + (i * 80),
      y: 150 + (i % 2) * 100,
      type: (room > 4 ? 'orc_warrior' : 'shadow_beast') as const
    }));
  }, [totalRooms]);
                ...
                >
                  ...
                  <AnimatePresence>
                    {enemyTelegraphs.map(telegraph => (
                      ...
                      >
                        <div className="w-full h-full border-4 border-red-500 rounded-full bg-red-500/20 animate-pulse">
